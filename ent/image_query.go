...
func (iq *ImageQuery) QueryUploadedBy() *UserQuery {
...
func (iq *ImageQuery) QueryBooks() *BookQuery {
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withUploadedBy: iq.withUploadedBy.Clone(),
		withBooks:      iq.withBooks.Clone(),
...
func (iq *ImageQuery) WithUploadedBy(opts ...func(*UserQuery)) *ImageQuery {
	query := (&UserClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withUploadedBy = query
	return iq
}
...
func (iq *ImageQuery) WithBooks(opts ...func(*BookQuery)) *ImageQuery {
	query := (&BookClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [2]bool{
			iq.withUploadedBy != nil,
			iq.withBooks != nil,
		}
	)
	if iq.withUploadedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withUploadedBy; query != nil {
		if err := iq.loadUploadedBy(ctx, query, nodes, nil,
			func(n *Image, e *User) { n.Edges.UploadedBy = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withBooks; query != nil {
		if err := iq.loadBooks(ctx, query, nodes,
			func(n *Image) { n.Edges.Books = []*Book{} },
			func(n *Image, e *Book) { n.Edges.Books = append(n.Edges.Books, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadUploadedBy(ctx context.Context, query *UserQuery, nodes []*Image, init func(*Image), assign func(*Image, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Image)
	for i := range nodes {
		if nodes[i].user_images == nil {
			continue
		}
		fk := *nodes[i].user_images
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_images" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *ImageQuery) loadBooks(ctx context.Context, query *BookQuery, nodes []*Image, init func(*Image), assign func(*Image, *Book)) error {
	...
	query.Where(predicate.Book(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.BooksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.image_books
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_books" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_books" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
