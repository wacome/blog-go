...
func (bq *BookQuery) QueryCoverImage() *ImageQuery {
...
func (bq *BookQuery) Clone() *BookQuery {
	...
	return &BookQuery{
		...
		withCoverImage: bq.withCoverImage.Clone(),
...
func (bq *BookQuery) WithCoverImage(opts ...func(*ImageQuery)) *BookQuery {
	query := (&ImageClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withCoverImage = query
	return bq
}
...
func (bq *BookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Book, error) {
	var (
		...
		_spec       = bq.querySpec()
		loadedTypes = [2]bool{
			bq.withCoverImage != nil,
			bq.withOwner != nil,
		}
	)
	if bq.withCoverImage != nil || bq.withOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := bq.withCoverImage; query != nil {
		if err := bq.loadCoverImage(ctx, query, nodes, nil,
			func(n *Book, e *Image) { n.Edges.CoverImage = e }); err != nil {
			return nil, err
		}
	}
...
func (bq *BookQuery) loadCoverImage(ctx context.Context, query *ImageQuery, nodes []*Book, init func(*Book), assign func(*Book, *Image)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Book)
	for i := range nodes {
		if nodes[i].image_books == nil {
			continue
		}
		fk := *nodes[i].image_books
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "image_books" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BookQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Book, init func(*Book), assign func(*Book, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Book)
	for i := range nodes {
