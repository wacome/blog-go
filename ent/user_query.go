...
func (uq *UserQuery) loadImages(ctx context.Context, query *ImageQuery, nodes []*User, init func(*User), assign func(*User, *Image)) error {
	...
	for _, n := range neighbors {
		fk := n.user_images
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_images" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_images" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
