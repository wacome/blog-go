...
// The HitokotoFunc type is an adapter to allow the use of ordinary
// function as Hitokoto mutator.
type HitokotoFunc func(context.Context, *ent.HitokotoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HitokotoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HitokotoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HitokotoMutation", m)
}
